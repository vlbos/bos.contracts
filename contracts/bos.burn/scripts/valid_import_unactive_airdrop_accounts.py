...
def validAccount(account,quantity):
    table = getJsonOutput(args.cleos + 'get table burn.bos '+account+' accounts -l 100')
    resCsv = []
    for row in table['rows']:
...
commands = [
    ...
    ('V', 'valid',              validAccounts,              True,    "Valid import accounts "),
...
parser.add_argument('--cleos', metavar='', help="Cleos command", default='/Users/lisheng/mygit/boscore/bos/build/programs/cleos/cleos --wallet-url http://127.0.0.1:6666 ')
...
# python3 valid_import_unactive_airdrop_accounts.py --cleos="cleos --wallet-url http://127.0.0.1:6666 " --nodeos=nodeos --keosd=keosd --contracts-dir="/EOSIO_CONTRACTS_DIRECTORY/" -a
