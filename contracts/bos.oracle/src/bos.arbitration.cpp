...
void bos_oracle::uploaddefer(name arbitrator, uint64_t arbitration_id, uint64_t process_id, uint64_t timer_type)
{
...
void bos_oracle::timertimeout(uint64_t arbitration_id, uint64_t timer_type)
{
    auto arbicaseapp_tb = arbicaseapps( get_self(), get_self().value );
    auto arbicaseapp_iter = arbicaseapp_tb.find(arbitration_id);
    check(arbicaseapp_iter != arbicaseapp_tb.end(), "Can not find such arbitration.");
    ...
    switch(timer_type)
    {
        ...
        default:
        {
            check(false, "unknown arbitration timer type.");
            break;
        }
    }
}
