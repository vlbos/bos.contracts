...
[[eosio::action]] 
void regservice(uint64_t service_id,name account,asset stake_amount,asset service_price,uint64_t fee_type,std::string data_format,
...
[[eosio::action]] 
void stakeamount(uint64_t service_id,uint64_t provider_id,name account,std::string signature,asset stake_amount);
...
[[eosio::action]] 
void subscribe(uint64_t service_id,name contract_account,name action_name,std::string publickey,name account,asset amount,
               std::string memo);
...
using payservice_action =
   ...
   ///
[[eosio::action]] 
void deposit(name from, name to, asset quantity,string memo,bool is_notify);
[[eosio::action]] 
...
void add_freeze_delay(uint64_t service_id, name account,time_point_sec start_time,
                            time_point_sec duration,asset amount, uint64_t status, uint64_t type) ;
uint64_t add_guarantee(uint64_t service_id, name account,time_point_sec start_time,
